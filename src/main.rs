...
use iced::{window, Application, Command, Element, Event, Font, Length, Renderer, Subscription};
...
use crate::bluetooth::{initialize_hci, DownloadFileProvider};
use crate::config::AppSettings;
use crate::states::{Running, SubState};
...
pub static PROJECT_DIRS: Lazy<ProjectDirs> =
    Lazy::new(|| ProjectDirs::from("com.github", "sidit77", "bluefang-player").expect("Failed to get config directories"));
...
fn log_file() -> File {
    let log_file = PROJECT_DIRS.data_local_dir().join("bluefang-player.log");
    std::fs::create_dir_all(log_file.parent().unwrap()).expect("Failed to create log directory");
...
fn main() -> iced::Result {
    ...
    Hci::register_firmware_loaders([RealTekFirmwareLoader::new(DownloadFileProvider {
...
impl Application for App {
    ...
    fn update(&mut self, message: Self::Message) -> Command<Self::Message> {
        match message {
            ...
            Message::CloseRequested => match &mut self.state {
            ...
            Message::Running(msg) => match &mut self.state {
                State::Running(running, _) => running.update(msg).map(Message::Running),
                _ => Command::none()
            },
    ...
    fn subscription(&self) -> Subscription<Self::Message> {
        let running_events = match &self.state {
            State::Running(running, _) => running.subscription().map(Message::Running),
            _ => Subscription::none()
        };
